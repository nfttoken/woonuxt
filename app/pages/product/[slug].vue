...
import type { LocalizedProductPayload } from '../../utils/aiCommerce';
...
const { data: localizedData } = await useFetch<LocalizedProductPayload>(() => `/api/products/${slug}`, {
  ...
  key: `localized-product:${slug}:${locale.value}`,
  default: () => null,
});
...
const applyLocalizedContent = () => {
  if (!product.value || !localizedData.value) return;

  product.value = {
    ...product.value,
    name: localizedData.value.title || product.value.name,
    description: localizedData.value.description || product.value.description,
    shortDescription: localizedData.value.description || product.value.shortDescription,
  };
};

watch([product, localizedData], applyLocalizedContent, { immediate: true });
...
const refreshStockStatus = async (): Promise<void> => {
  try {
    ...
    const errorMessage = stockError?.gqlErrors?.[0]?.message;
    ...
    <div v-if="product">
      ...
      <div class="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_minmax(26rem,34rem)] lg:gap-24">
        ...
        <div class="w-full min-w-0 md:py-2">
          ...
          <div class="mb-8 font-light prose" v-html="localizedDescription"></div>

...
const props = defineProps({
  node: { type: Object as PropType<ProductWithMeta>, required: true },
  index: { type: Number, default: 1 },
});

const localized = useProductTranslation(computed(() => props.node));
const displayName = computed(() => localized.value.title || props.node?.name || '');
...
const sliderImages = computed<ProductImage[]>(() => {
  ...
  const addVariationImage = (variation: ProductVariationFragment) => {
    ...
    addImage({
      src,
      alt: variation?.image?.altText || displayName.value || 'Product image',
      title: variation?.image?.title || displayName.value || 'Product image',
  ...
  const addGalleryImage = (image: ImageFragment) => {
    ...
    addImage({
      src: image.sourceUrl,
      alt: image?.altText || displayName.value || 'Product image',
      title: image?.title || displayName.value || 'Product image',
  ...
  const main = {
    src: mainImage.value,
    alt: props.node?.image?.altText || displayName.value || 'Product image',
    title: props.node?.image?.title || displayName.value || 'Product image',
...
<template>
  <div class="relative group">
    ...
    <div class="p-2">
      ...
      <NuxtLink v-if="node.slug" :to="productLink" :title="displayName || undefined">
        <h2 class="mb-2 font-light leading-tight text-gray-900 group-hover:text-primary">{{ displayName }}</h2>
      </NuxtLink>
